...
import { type TeamState } from '@/hooks/use-shared-state'
...
import {
  ...
  Phone, MapPin, Building, Zap, ScanEye, Brain,
  Users, Crosshair,
} from 'lucide-react'
...
export default function AuthGate() {
  ...
  // ── Fetch teams once for squad status (no heavy polling on login page) ──
  const [sharedTeams, setSharedTeams] = useState<TeamState[]>([])

  useEffect(() => {
    // Only fetch teams when in rescue-squad-select mode
    if (mode !== 'rescue-squad-select') return
    ...
    const fetchTeams = async () => {
      try {
        ...
        if (!cancelled) setSharedTeams(data.teams ?? [])
      } catch {
        // Silently fail - static squad data will be used
      }
    }
    fetchTeams()
    // Poll every 5 seconds (not 800ms) — only when on squad select screen
  ...
  function handleTrackSelect(track: 'public' | 'rescue' | 'admin') {
    if (track === 'public') {
      ...
      toast.success(tAuth('welcomeAdmin'), { description: tAuth('welcomeAdminDesc') })
    }
  }

  function handleSquadLogin() {
    if (!selectedSquadId) {
      toast.error(tAuth('noSquadSelected'), { description: tAuth('noSquadDesc') })
      return
    }
    ...
    setAuthenticated(true)
    toast.success(
      tAuth('welcomeSquad', { name: squad.name }),
      { description: tAuth('welcomeSquadDesc', { type: squad.unitType, status: squad.status }) }
    )
  }
  ...
  function handleCitizenLogin(e: React.FormEvent) {
    ...
    if (!loginName.trim() || !loginPhone.trim()) {
      toast.error(tAuth('missingFields'), { description: tAuth('missingFieldsDesc') })
      return
    }
    setRole('public')
    ...
    setAuthenticated(true)
    toast.success(
      tAuth('welcomeCitizen', { name: loginName.trim() }),
      { description: tAuth('welcomeCitizenDesc') }
    )
  }
  ...
  function handleCitizenSignup(e: React.FormEvent) {
    ...
    if (!signupName.trim() || !signupPhone.trim() || !signupDistrict) {
      toast.error(tAuth('missingAllFields'), { description: tAuth('missingAllFieldsDesc') })
      return
    }
    setRole('public')
    ...
    setAuthenticated(true)
    toast.success(
      tAuth('welcomeCitizen', { name: signupName.trim() }),
      { description: tAuth('welcomeCitizenDesc') }
    )
  }

  const featureCards = [
    { icon: Zap, title: tFeatures('situationalAwareness'), desc: tFeatures('situationalDesc') },
    { icon: ScanEye, title: tFeatures('verification'), desc: tFeatures('verificationDesc') },
    { icon: Brain, title: tFeatures('aiIntelligence'), desc: tFeatures('aiDesc') },
  ]
        ...
        <div className="hidden lg:flex flex-col justify-center w-1/2 p-12 xl:p-16 relative">
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold tracking-tight">{tApp('title')}</h1>
                <p className="text-sm text-slate-400 mt-0.5">{tApp('tagline')}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/15 border border-red-500/30 text-sm">
                ...
                <span className="text-red-300 font-semibold">{tApp('eventBadge')}</span>
              </div>
              <p className="text-lg text-slate-200">{tApp('eventLocation')}</p>
              <p className="text-sm text-slate-400">{tApp('eventDesc')}</p>
            </div>
          ...
          <div className="w-full max-w-md space-y-6">
            ...
            <div className="lg:hidden text-center space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{tApp('tagline')}</p>
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/15 border border-red-500/30 text-xs">
                ...
                <span className="text-red-300 font-semibold">{tApp('eventBadge')} — Latur, MH</span>
              </div>
            </div>
            ...
            {mode === 'track-select' && (
              <div className="space-y-6 portal-enter">
                <div className="text-center space-y-1.5">
                  <h2 className="text-xl font-bold">{tAuth('selectTrack')}</h2>
                  <p className="text-sm text-slate-400">{tAuth('selectTrackDesc')}</p>
                </div>
                ...
                <div className="grid gap-3">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-semibold text-emerald-300">{tAuth('citizenTitle')}</h3>
                          <ArrowRight className="h-4 w-4 text-emerald-400/60 group-hover:translate-x-1 transition-transform shrink-0" />
                        </div>
                        <p className="text-xs text-slate-400 mt-1">{tAuth('citizenDesc')}</p>
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-semibold text-amber-300">{tAuth('rescueTitle')}</h3>
                          <ArrowRight className="h-4 w-4 text-amber-400/60 group-hover:translate-x-1 transition-transform shrink-0" />
                        </div>
                        <p className="text-xs text-slate-400 mt-1">{tAuth('rescueDesc')}</p>
                      </div>
                    </div>
                  </button>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-semibold text-red-300">{tAuth('adminTitle')}</h3>
                          <ArrowRight className="h-4 w-4 text-red-400/60 group-hover:translate-x-1 transition-transform shrink-0" />
                        </div>
                        <p className="text-xs text-slate-400 mt-1">{tAuth('adminDesc')}</p>
                      </div>
                    </div>
                  </button>
                </div>
                ...
                {/* Demo quick access */}
                <div className="pt-2">
                  <p className="text-[11px] text-slate-500 text-center uppercase tracking-wider font-medium mb-3">{tAuth('demoQuickAccess')}</p>
                  <div className="flex gap-2 justify-center">
                    <Button variant="outline" size="sm" className="text-[11px] h-7 px-2.5 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 hover:text-emerald-300" onClick={() => handleTrackSelect('public')}>
                      <Globe className="h-3 w-3 mr-1" /> {tAuth('citizen')}
                    </Button>
                    <Button variant="outline" size="sm" className="text-[11px] h-7 px-2.5 border-amber-500/30 text-amber-400 hover:bg-amber-500/10 hover:text-amber-300" onClick={() => handleTrackSelect('rescue')}>
                      <Shield className="h-3 w-3 mr-1" /> {tAuth('rescue')}
                    </Button>
                    <Button variant="outline" size="sm" className="text-[11px] h-7 px-2.5 border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300" onClick={() => handleTrackSelect('admin')}>
                      <Terminal className="h-3 w-3 mr-1" /> {tAuth('admin')}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            {mode === 'rescue-squad-select' && (
              <div className="space-y-5 portal-enter">
                <button onClick={() => setMode('track-select')} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-200 transition-colors">
                  {tAuth('backToTrack')}
                </button>
                <Card className="border-white/10 bg-white/5 backdrop-blur-sm">
                  <CardHeader className="pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-lg text-white">{tAuth('squadAuth')}</CardTitle>
                        <p className="text-xs text-slate-400 mt-0.5">{tAuth('squadAuthDesc')}</p>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={selectedSquadId} onValueChange={setSelectedSquadId}>
                        <SelectTrigger className="w-full bg-white/5 border-white/15 text-white focus:ring-amber-500/50 focus:border-amber-500/50">
                          <SelectValue placeholder={tAuth('squadPlaceholder')} />
                        </SelectTrigger>
                        <SelectContent className="bg-slate-900 border-white/15">
                          {SQUAD_OPTIONS.map((squad) => (
                            <SelectItem key={squad.id} value={squad.id} className="text-slate-200 focus:bg-amber-500/15 focus:text-amber-200">
                              <div className="flex items-center gap-2">
                      ...
                      <div className="rounded-lg bg-white/5 border border-white/10 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <span className="text-sm font-semibold text-amber-300">{selectedSquad.name}</span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[11px]">
                          <div className="text-slate-400">{tAuth('unitType')}: <span className="text-slate-200">{selectedSquad.unitType}</span></div>
                          <div className="text-slate-400">{tAuth('members')}: <span className="text-slate-200">{selectedSquad.members}</span></div>
                          <div className="text-slate-400 col-span-2">{tAuth('position')}: <span className="text-slate-200 font-mono">{selectedSquad.lat?.toFixed(4) ?? '—'}°N, {selectedSquad.lng?.toFixed(4) ?? '—'}°E</span></div>
                        </div>
                        {selectedSquad.assignedIncidentId && (
                          <div className="text-[11px] text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded px-3 py-2">
                            {tAuth('activeAssignment')}
                          </div>
                        )}
                      </div>
                    )}

                    <Button className="w-full bg-amber-600 hover:bg-amber-500 text-white border-0" onClick={handleSquadLogin} disabled={!selectedSquadId}>
                ...
                <Card className="border-white/10 bg-white/5 backdrop-blur-sm">
                  <CardHeader className="pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-lg text-white">{tAuth('citizenLogin')}</CardTitle>
                        <p className="text-xs text-slate-400 mt-0.5">{tAuth('citizenLoginDesc')}</p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <form onSubmit={handleCitizenLogin} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Input id="login-name" placeholder={tAuth('namePlaceholder')} value={loginName} onChange={(e) => setLoginName(e.target.value)} className="bg-white/5 border-white/15 text-white placeholder:text-slate-500 focus-visible:border-emerald-500/50" />
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input id="login-phone" type="tel" placeholder={tAuth('phonePlaceholder')} value={loginPhone} onChange={(e) => setLoginPhone(e.target.value)} className="bg-white/5 border-white/15 text-white placeholder:text-slate-500 focus-visible:border-emerald-500/50 pl-12" />
                        </div>
                      </div>
                      <Button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white border-0">
                        <LogIn className="h-4 w-4 mr-2" />
                        {tAuth('loginAsCitizen')}
                      </Button>
                    </form>
                    <div className="mt-4 text-center">
                      <button onClick={() => { setLoginName(''); setLoginPhone(''); setMode('citizen-signup') }} className="text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
                        <UserPlus className="h-3 w-3 mr-1 inline" />
                        {tAuth('registerAsCitizen')}
                      </button>
                    </div>
                  </CardContent>
                </Card>
            ...
            {mode === 'citizen-signup' && (
              <div className="space-y-5 portal-enter">
                <button onClick={() => { setSignupName(''); setSignupPhone(''); setSignupDistrict(''); setMode('track-select') }} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-200 transition-colors">
                  {tAuth('backToTrack')}
                </button>
                <Card className="border-white/10 bg-white/5 backdrop-blur-sm">
                  <CardHeader className="pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-lg text-white">{tAuth('citizenSignup')}</CardTitle>
                        <p className="text-xs text-slate-400 mt-0.5">{tAuth('citizenSignupDesc')}</p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <form onSubmit={handleCitizenSignup} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Input id="signup-name" placeholder={tAuth('enterFullName')} value={signupName} onChange={(e) => setSignupName(e.target.value)} className="bg-white/5 border-white/15 text-white placeholder:text-slate-500 focus-visible:border-emerald-500/50" />
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-slate-400">+91</span>
                          <Input id="signup-phone" type="tel" placeholder={tAuth('phonePlaceholder')} value={signupPhone} onChange={(e) => setSignupPhone(e.target.value)} className="bg-white/5 border-white/15 text-white placeholder:text-slate-500 focus-visible:border-emerald-500/50 pl-12" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        ...
                        <Select value={signupDistrict} onValueChange={setSignupDistrict}>
                          <SelectTrigger className="w-full bg-white/5 border-white/15 text-white focus:ring-emerald-500/50 focus:border-emerald-500/50">
                            <SelectValue placeholder={tAuth('selectDistrict')} />
                          </SelectTrigger>
                          <SelectContent className="bg-slate-900 border-white/15">
                            {MAHARASHTRA_DISTRICTS.map((d) => (
                              <SelectItem key={d} value={d} className="text-slate-200 focus:bg-emerald-500/15 focus:text-emerald-200">{d}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
                    ...
                    <div className="mt-4 text-center">
                      <button onClick={() => { setSignupName(''); setSignupPhone(''); setSignupDistrict(''); setMode('citizen-login') }} className="text-xs text-slate-400 hover:text-slate-200 transition-colors">
