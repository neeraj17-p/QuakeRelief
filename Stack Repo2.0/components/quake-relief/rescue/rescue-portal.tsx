...
/** Generate a circle polygon (array of [lat, lng] points) for hazard zone avoidance */
...
export default function RescuePortal() {
  ...
  const routingQueries = useMemo(() => {
    ...
    const hazardPolygons: [number, number][][] = []
    // Generate hazard exclusion polygons from active incidents + epicenter
    const center = selfPosition || MAP_CENTER
    const activeIncidentsWithCircles = incidents.filter(inc =>
      inc.status !== 'RESOLVED' && inc.priority && PRIORITY_RADIUS[inc.priority] > 0
    )
    for (const inc of activeIncidentsWithCircles) {
      const polygon = generateCirclePolygon(
        [inc.latitude, inc.longitude],
        PRIORITY_RADIUS[inc.priority] || 75,
        40,
      )
      if (hazardPolygons.length < 20) hazardPolygons.push(polygon)
    }
    // Also add epicenter as a P1 hazard zone
    const epicPoly = generateCirclePolygon(
      [EPICENTRE.lat, EPICENTRE.lng], 500, 40,
    )
    if (hazardPolygons.length < 20) hazardPolygons.push(epicPoly)
    return [
      {
        id: 'my-route',
        from: selfPosition,
