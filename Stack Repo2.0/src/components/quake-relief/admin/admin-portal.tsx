...
import { broadcastVoiceAlert } from '@/hooks/use-voice-alert-listener'
...
import {
  ...
  ChevronDown, ChevronUp, Navigation, FileText, FileSpreadsheet, Download, Megaphone, Square,
  Sparkles, Skull, Flame, Ban, LandPlot, Droplets, Building2, Siren, Gavel,
} from 'lucide-react'
...
// Per-entry triage cache keyed by verification entry id
interface EntryTriageCache {
  [entryId: string]: TriageResult | null
}
...
const HAZARD_CONFIG: Record<string, { icon: React.ReactNode; color: string; bg: string; label: string }> = {
  STRUCTURAL_COLLAPSE: { icon: <Building2 className="h-4 w-4" />, color: 'text-red-400', bg: 'bg-red-500/15 border-red-500/30', label: 'Structural Collapse' },
  MEDICAL_EMERGENCY: { icon: <Activity className="h-4 w-4" />, color: 'text-pink-400', bg: 'bg-pink-500/15 border-pink-500/30', label: 'Medical Emergency' },
  FIRE: { icon: <Flame className="h-4 w-4" />, color: 'text-orange-400', bg: 'bg-orange-500/15 border-orange-500/30', label: 'Fire' },
  ROAD_BLOCK: { icon: <Ban className="h-4 w-4" />, color: 'text-amber-400', bg: 'bg-amber-500/15 border-amber-500/30', label: 'Road Block' },
  LANDSLIDE: { icon: <LandPlot className="h-4 w-4" />, color: 'text-yellow-400', bg: 'bg-yellow-500/15 border-yellow-500/30', label: 'Landslide' },
  FLOOD: { icon: <Droplets className="h-4 w-4" />, color: 'text-cyan-400', bg: 'bg-cyan-500/15 border-cyan-500/30', label: 'Flood' },
  SPAM_OTHER: { icon: <Siren className="h-4 w-4" />, color: 'text-slate-400', bg: 'bg-slate-500/15 border-slate-500/30', label: 'Spam / Other' },
}

const SEVERITY_CONFIG: Record<string, { color: string; bg: string; label: string; pulse: boolean }> = {
  P1_CRITICAL: { color: 'text-red-400', bg: 'bg-red-500/20 border-red-500/40', label: 'P1 CRITICAL', pulse: true },
  P2_HIGH: { color: 'text-orange-400', bg: 'bg-orange-500/20 border-orange-500/40', label: 'P2 HIGH', pulse: false },
  P3_MEDIUM: { color: 'text-amber-400', bg: 'bg-amber-500/20 border-amber-500/40', label: 'P3 MEDIUM', pulse: false },
  P4_LOW: { color: 'text-emerald-400', bg: 'bg-emerald-500/20 border-emerald-500/40', label: 'P4 LOW', pulse: false },
}
...
export default function AdminPortal() {
  ...
  // ── Voice Broadcast state (Web Speech API — zero dependencies, fully offline) ──
  ...
  const [entryTriageCache, setEntryTriageCache] = useState<EntryTriageCache>({})
  const [triagingEntryId, setTriagingEntryId] = useState<string | null>(null)

  // ── Speak text locally using Web Speech API with Indian accent voice matching ──
  ...
  const handleVoiceBroadcast = useCallback(() => {
    ...
    const text = broadcastHeader.trim()
      ? `${broadcastHeader}. ${broadcastMsg}`
      : broadcastMsg.trim()

    if (!text) return

    speakText(text, voiceLang)
    const langName = voiceLang.split('-')[0] === 'en' ? 'English' : voiceLang.split('-')[0] === 'mr' ? 'Marathi' : 'Hindi'
    toast.success(`Broadcasting voice alert in ${langName}`)
  }, [broadcastHeader, broadcastMsg, voiceLang, isSpeaking, speakText])

  // ── Voice + Text Alert: broadcasts text AND triggers cross-portal voice ──
  const handleVoiceAndTextBroadcast = useCallback(async () => {
    const text = broadcastHeader.trim()
      ? `${broadcastHeader}. ${broadcastMsg}`
      : broadcastMsg.trim()

    if (!text || !broadcastHeader.trim()) {
      toast.error('Please enter a header and message for voice + text alert')
      return
    }

    setSendingBroadcast(true)
    try {
      // 1. Send text broadcast via API
      await fetch('/api/broadcast', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: broadcastMsg.trim(),
          header: broadcastHeader.trim(),
      ...
      // 2. Speak locally in SEOC portal
      speakText(text, voiceLang)
      // 3. Broadcast voice alert to all other open portals (Rescue, Citizen)
      broadcastVoiceAlert(text, voiceLang)
      toast.success(`Voice + Text alert broadcast in ${voiceLang.split('-')[0] === 'en' ? 'English' : voiceLang.split('-')[0] === 'mr' ? 'Marathi' : 'Hindi'}`)
    } catch {
      toast.error('Failed to send broadcast')
    } finally {
      setSendingBroadcast(false)
    }
  }, [broadcastHeader, broadcastMsg, broadcastSeverity, broadcastChannel, voiceLang, speakText])
  ...
  const verifiedInPipelineCount = verifications.filter(v => v.status === 'VERIFIED').length
  ...
  const handleExport = useCallback((format: 'pdf' | 'csv') => {
    ...
    link.href = `/api/export?format=${format}&eventId=eq-maharashtra-2025-001`
    const dateStr = new Date().toISOString().slice(0, 10)
    link.download = format === 'pdf' ? `sitrep-${dateStr}.html` : `incidents-${dateStr}.csv`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    setTimeout(() => setExporting(null), 1500)
    toast.success(`${format === 'pdf' ? 'SITREP report' : 'CSV data'} exported successfully`)
  }, [])

  // ── Verification pipeline tab state ──
  const [pipelineTab, setPipelineTab] = useState<'active' | 'junk'>('active')
  // ── Override priority state ──
  const [overridingId, setOverridingId] = useState<string | null>(null)

  // ── Sorted verifications — ALL items visible (verified stay with green badge) ──
  const sortedVerifications = useMemo(() => {
    const sorted = [...verifications].sort((a, b) => {
      // Push VERIFIED items to bottom, FALSE items to very bottom
      const statusOrder: Record<string, number> = { VERIFIED: 1, FALSE: 2 }
      const aStatus = statusOrder[a.status] ?? 0
      const bStatus = statusOrder[b.status] ?? 0
      if (aStatus !== bStatus) return aStatus - bStatus
      const triA = entryTriageCache[a.id]
      const triB = entryTriageCache[b.id]
      if (triA && triB) {
        const order: Record<string, number> = { P1_CRITICAL: 0, P2_HIGH: 1, P3_MEDIUM: 2, P4_LOW: 3 }
        return (order[triA.severity_tier] ?? 9) - (order[triB.severity_tier] ?? 9)
      }
      if (triA) return -1
      if (triB) return 1
      return 0
    ...
    return sorted
  }, [verifications, entryTriageCache])

  const junkVerifications = useMemo(() => {
    return verifications.filter(v => entryTriageCache[v.id]?.is_spam)
  }, [verifications, entryTriageCache])

  // ── Handle priority override ──
  const handleOverridePriority = useCallback((entryId: string, newTier: string) => {
    setEntryTriageCache(prev => {
      const existing = prev[entryId]
      if (!existing) return prev
      return { ...prev, [entryId]: { ...existing, severity_tier: newTier as any } }
    })
    setOverridingId(null)
    toast.success(`Priority overridden to ${newTier}`)
  }, [])

  // ── AI Triage handler ──
  const handleTriage = useCallback(async (reportText: string) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        toast.success(`AI Triage complete in ${data.data.processing_time_ms}ms — ${data.data.severity_tier}`)
      } else {
        toast.error(data.error || 'Triage analysis failed')
      }
    } catch (err: any) {
      toast.error(err?.message || 'Triage request failed')
    } finally {
      setTriaging(false)
    }
  }, [])

  // ── Per-entry triage (from verification pipeline) ──
  const handleEntryTriage = useCallback(async (entryId: string, rawContent: string) => {
    setTriagingEntryId(entryId)
    try {
      const res = await fetch('/api/triage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ report: rawContent }),
      })
      const data = await res.json()
      if (data.success && data.data) {
        setEntryTriageCache(prev => ({ ...prev, [entryId]: data.data }))
        setTriageResult(data.data)
        toast.success(`Entry triaged: ${data.data.severity_tier}`)
      } else {
        toast.error(data.error || 'Entry triage failed')
      }
    } catch (err: any) {
      toast.error(err?.message || 'Entry triage request failed')
    } finally {
      setTriagingEntryId(null)
    }
  }, [])
  ...
  return (
    <div className="relative block w-full min-h-screen bg-slate-900 text-white overflow-y-auto px-4 sm:px-6 py-8 flex flex-col gap-y-10 pb-24">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              size="sm"
              variant="outline"
              className="border-slate-700 bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white gap-2"
      ...
      {/* ═══ 2. AI TRIAGE ENGINE ═══ */}
      <section aria-label="AI Triage Engine">
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 break-words whitespace-normal normal-case block w-full max-w-full overflow-hidden">
            AI Report Triage Engine
          </h2>
          <Badge variant="outline" className="gap-1.5 px-2.5 py-0.5 border-violet-500/40 bg-violet-500/10 text-violet-400 text-[10px] font-bold">
            <Brain className="h-3 w-3" /> LLM-Powered
          </Badge>
        </div>
        ...
        <div className="bg-slate-800/80 border border-slate-700/50 rounded-xl p-5">
          ...
          <div className="flex gap-3 mb-4">
            <Textarea
              value={triageInput}
              onChange={e => setTriageInput(e.target.value.slice(0, 5000))}
              placeholder="Paste a raw citizen report for AI triage analysis..."
              className="flex-1 min-h-[72px] max-h-28 resize-none bg-slate-900/60 border-slate-600/50 text-white placeholder:text-slate-500 text-sm"
              rows={2}
            />
            <Button
              onClick={() => handleTriage(triageInput)}
              disabled={!triageInput.trim() || triaging}
              className="shrink-0 bg-violet-600 hover:bg-violet-700 text-white gap-2 h-auto px-5"
            >
              {triaging ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
          ...
          <div className="flex justify-end mb-1">
            <span className={`text-[10px] font-mono ${triageInput.length > 4500 ? 'text-red-400 font-bold' : 'text-slate-500'}`}>
              {triageInput.length} / 5000
            </span>
          </div>

          {/* Result card */}
          {triageResult && (() => {
            const hazCfg = HAZARD_CONFIG[triageResult.hazard_category] || HAZARD_CONFIG.SPAM_OTHER
            const sevCfg = SEVERITY_CONFIG[triageResult.severity_tier] || SEVERITY_CONFIG.P3_MEDIUM
            return (
              <div className="mt-4 border border-slate-600/50 rounded-xl overflow-hidden">
                {/* Result header */}
                <div className="bg-slate-900/80 px-4 py-3 flex items-center justify-between border-b border-slate-700/50">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${hazCfg.bg} border`}>
                      {hazCfg.icon}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-bold ${hazCfg.color}`}>{hazCfg.label}</span>
                        <Badge variant="outline" className={`text-[10px] border font-bold ${sevCfg.bg} ${sevCfg.color} ${sevCfg.pulse ? 'animate-pulse' : ''}`}>
                          {sevCfg.label}
                        </Badge>
                      </div>
                      <div className="text-[10px] text-slate-500 mt-0.5">Processed in {triageResult.processing_time_ms}ms</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {/* Confidence gauge */}
                    <div className="flex items-center gap-1.5">
                      <div className="w-16 h-2 rounded-full bg-slate-700 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${triageResult.confidence_score * 100}%`,
                            background: triageResult.confidence_score > 0.8 ? '#22c55e' : triageResult.confidence_score > 0.5 ? '#f59e0b' : '#ef4444',
                          }}
                        />
                      </div>
                      <span className="text-[10px] font-bold text-slate-300">{Math.round(triageResult.confidence_score * 100)}%</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Operational Summary</div>
                    <div className="text-sm text-slate-200 leading-relaxed">{triageResult.summary}</div>
                  </div>

                  {/* Metrics row */}
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <Skull className={`h-4 w-4 ${triageResult.casualty_estimate > 0 ? 'text-red-400' : 'text-slate-500'}`} />
                      <div>
                        <div className={`text-lg font-bold ${triageResult.casualty_estimate > 0 ? 'text-red-400' : 'text-slate-400'}`}>{triageResult.casualty_estimate}</div>
                        <div className="text-[10px] text-slate-500">Est. Casualties</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <UserCheck className={`h-4 w-4 ${triageResult.trapped_victims ? 'text-red-400' : 'text-slate-500'}`} />
                      <div>
                        <div className={`text-lg font-bold ${triageResult.trapped_victims ? 'text-red-400' : 'text-slate-400'}`}>{triageResult.trapped_victims ? 'YES' : 'NO'}</div>
                        <div className="text-[10px] text-slate-500">Trapped Victims</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Gavel className={`h-4 w-4 ${triageResult.is_spam ? 'text-amber-400' : 'text-emerald-400'}`} />
                      <div>
                        <div className={`text-lg font-bold ${triageResult.is_spam ? 'text-amber-400' : 'text-emerald-400'}`}>{triageResult.is_spam ? 'FLAGGED' : 'CLEAN'}</div>
                        <div className="text-[10px] text-slate-500">Spam Check</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900/60 border border-slate-700/40 rounded-lg p-3">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5 flex items-center gap-1.5">
                      <Navigation className="h-3 w-3" /> Suggested Action
                    </div>
                    <div className="text-xs text-slate-200 leading-relaxed">{triageResult.suggested_action}</div>
                  </div>
                </div>
              </div>
            )
      ...
      <section aria-label="Data Verification Pipeline">
        ...
        {/* Tab bar: Active Queue | Filtered / Junk */}
        <div className="flex items-center gap-2 mb-3">
          <button
            onClick={() => setPipelineTab('active')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${pipelineTab === 'active' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200'}`}
          >
            Active Queue ({sortedVerifications.length})
          </button>
          {junkVerifications.length > 0 && (
            <button
              onClick={() => setPipelineTab('junk')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${pipelineTab === 'junk' ? 'bg-red-600/80 text-white' : 'bg-slate-800 text-slate-500 hover:bg-slate-700 hover:text-slate-300'}`}
            >
              Filtered / Junk ({junkVerifications.length})
            </button>
          )}
        </div>

        <div className="w-full max-h-[420px] overflow-y-auto flex flex-col gap-y-3 relative block bg-slate-800/80 border border-slate-700/50 rounded-xl p-4 custom-scrollbar">
          {loading ? (
          ...
          ) : pipelineTab === 'junk' && junkVerifications.length === 0 ? (
            <div className="text-sm text-slate-500">No spam-filtered entries.</div>
          ) : pipelineTab === 'active' && sortedVerifications.length === 0 && verifications.length === 0 ? (
            <div className="text-sm text-slate-500">No verification entries loaded.</div>
          ) : (
            (pipelineTab === 'junk' ? junkVerifications : sortedVerifications).map(v => {
              const isActionable = v.status !== 'VERIFIED' && v.status !== 'FALSE'
              const tri = entryTriageCache[v.id]
              return (
                <div
                  key={v.id}
                  className={`w-full rounded-lg border p-3 transition-all ${tri?.is_spam ? 'border-slate-700/30 bg-slate-800/30 opacity-60' : 'border-slate-700/50 bg-slate-900/40 hover:border-slate-600/50'}`}
                >
                  {/* Row 1: Badges + raw text + action buttons */}
                  <div className="flex items-center gap-3 w-full">
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <div className="flex items-center gap-1.5 min-w-[60px]">
                        <div className="flex-1 h-1.5 rounded-full bg-slate-700 overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-500" style={{ width: `${(v.confidence * 100)}%`, background: v.confidence > 0.8 ? '#22c55e' : v.confidence > 0.5 ? '#f59e0b' : '#ef4444' }} />
                        </div>
                    ...
                    {/* AI Severity badge */}
                    {tri && (() => {
                      const sevCfg = SEVERITY_CONFIG[tri.severity_tier]
                      return sevCfg ? (
                        <Badge variant="outline" className={`text-[9px] border font-bold shrink-0 ${sevCfg.bg} ${sevCfg.color} gap-1 ${sevCfg.pulse ? 'animate-pulse' : ''}`}>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => handleEntryTriage(v.id, v.rawContent)}
                        disabled={triagingEntryId === v.id}
                        className="shrink-0 px-2.5 py-1.5 rounded-lg bg-violet-600/20 border border-violet-500/40 text-violet-400 text-[10px] font-semibold hover:bg-violet-600/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Run AI Triage"
                      >
                        {triagingEntryId === v.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Brain className="h-3 w-3" />}
                        AI
                      </button>
                      {isActionable && (
                        <>
                                          <button onClick={() => handleVerify(v.id)} className="shrink-0 px-2.5 py-1.5 rounded-lg bg-emerald-600/20 border border-emerald-500/40 text-emerald-400 text-[10px] font-semibold hover:bg-emerald-600/30 transition-colors">
                            ✔️ Verify
                          </button>
                          <button onClick={() => handleReject(v.id)} className="shrink-0 px-2.5 py-1.5 rounded-lg bg-red-600/20 border border-red-500/40 text-red-400 text-[10px] font-semibold hover:bg-red-600/30 transition-colors">
                            Reject
                          </button>
                          {tri && (
                            <div className="relative">
                              <button onClick={() => setOverridingId(overridingId === v.id ? null : v.id)} className="shrink-0 px-2.5 py-1.5 rounded-lg bg-amber-600/20 border border-amber-500/40 text-amber-400 text-[10px] font-semibold hover:bg-amber-600/30 transition-colors flex items-center gap-1">
                                <Gavel className="h-3 w-3" />
                                Override
                              </button>
                              {overridingId === v.id && (
                                <div className="absolute top-full right-0 mt-1 z-50 bg-slate-900/95 backdrop-blur-sm border border-slate-600/60 rounded-lg shadow-2xl shadow-black/40 overflow-hidden">
                                  {['P1_CRITICAL', 'P2_HIGH', 'P3_MEDIUM', 'P4_LOW'].map(tier => {
                                    const cfg = SEVERITY_CONFIG[tier]
                                    return (
                                      <button key={tier} onClick={() => handleOverridePriority(v.id, tier)} className={`w-full text-left px-3 py-2 hover:bg-slate-800/80 border-b border-slate-700/30 last:border-0 transition-colors text-xs font-semibold ${cfg?.color || 'text-slate-400'}`}>
                                        {cfg?.label || tier}
                                      </button>
                                    )
                                  })}
                  ...
                  {/* Row 2: AI enriched metadata (only if triaged) */}
                  {tri && !tri.is_spam && (
                    <div className="flex items-center gap-4 mt-2 pl-1">
                      <span className="text-[10px] font-semibold text-violet-400 shrink-0">AI Confidence: {Math.round(tri.confidence_score * 100)}%</span>
                      <span className="text-[10px] text-slate-400">{tri.summary}</span>
                      <span className="text-[10px] text-cyan-400 shrink-0">→ {tri.suggested_action}</span>
                    </div>
                  )}
                </div>
              )
      ...
      <section aria-label="Command Broadcast">
        <div className="bg-slate-800/80 border border-slate-700/50 rounded-xl p-5">
          ...
          {/* Voice + Text Alert | Text-Only Send buttons */}
          <div className="flex items-center justify-between mt-4 gap-3">
            {/* Language selector + Voice + Text Alert (highlighted warning style) */}
            <div className="flex items-center gap-2 shrink-0">
              <select
                value={voiceLang}
                onChange={e => setVoiceLang(e.target.value)}
                className="h-9 text-[11px] bg-slate-900/60 border border-slate-600/50 text-slate-300 rounded-md px-2 focus:outline-none focus:ring-1 focus:ring-amber-500/50"
              >
              ...
              <Button
                onClick={handleVoiceAndTextBroadcast}
                disabled={!broadcastHeader.trim() || !broadcastMsg.trim() || sendingBroadcast}
                className="shrink-0 bg-amber-600 hover:bg-amber-700 text-white gap-2 h-auto px-4 shadow-lg shadow-amber-500/20 animate-pulse"
              >
                {sendingBroadcast ? <Loader2 className="h-4 w-4 animate-spin" /> : <Megaphone className="h-4 w-4" />}
                <span>Voice + Text Alert</span>
              </Button>
            </div>
